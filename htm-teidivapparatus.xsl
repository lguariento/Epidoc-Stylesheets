...
<!-- $Id$ --><xsl:stylesheet xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:t="http://www.tei-c.org/ns/1.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="#all" version="2.0">
    ...
    <xsl:template match="t:div[@type = 'apparatus']" priority="1">
        ...
        <!-- Removed redundant <div id="apparatus"> -->
            <xsl:choose>
                <xsl:when test="$parm-edn-structure='ogham'">
                    <h4>Critical Apparatus</h4>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template match="t:div[@type = 'apparatus']//t:app">
        ...
        <xsl:choose>
            <xsl:when test="$parm-edn-structure='ogham' and @n != following-sibling::t:app[1]/@n">
                <br/>
            </xsl:when>
            <xsl:when test="$parm-edn-structure!='ogham' and @loc != following-sibling::t:app[1]/@loc">
                <br/>
            </xsl:when>
                ...
                <xsl:for-each select="$biblio">
                    ...
                    <xsl:variable name="bibl">
                        <xsl:if test="doc-available($bibliography-al)">
                            <xsl:sequence select="document($bibliography-al)//t:bibl[@xml:id = $bib][not(@sameAs)]"/>
                        </xsl:if>
                    </xsl:variable>
                    <xsl:if test="position() = 1">
                        <xsl:text> </xsl:text>
