...
<!-- $Id$ --><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i18n="http://apache.org/cocoon/i18n/2.1" xmlns:t="http://www.tei-c.org/ns/1.0" exclude-result-prefixes="t" version="2.0">
    <!-- Contains named templates for ogham file structure -->
    <!-- Called from htm-tpl-structure.xsl -->
    <xsl:param name="nav-prev" select="'disable'"/>
    <xsl:param name="nav-next" select="'disable'"/>
    <xsl:template name="ogham-title">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:text>EpiDoc output, Og(h)am style</xsl:text>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>


    <xsl:template name="ogham-structure">
        <xsl:variable name="title">
            <xsl:call-template name="ogham-title"/>
        </xsl:variable>

        <xsl:variable name="previous" select="$nav-prev"/>
        <xsl:variable name="next" select="$nav-next"/>
        ...
        <html>
            ...
            <body>
                ...
                <!-- Add CIIC number with line break if it exists -->
                <xsl:if test="normalize-space(string(//t:publicationStmt/t:idno[@type='CIIC']))">
                    <h2>
                        CIIC <xsl:value-of select="//t:publicationStmt/t:idno[@type='CIIC']"/>
                    </h2>
                </xsl:if>
                ...
                <xsl:call-template name="ogham-body-structure"/>
            </body>
        </html>
    </xsl:template>

    <xsl:template name="ogham-body-structure">
        <div id="metadata">
            ...
            <div class="row align-items-start">
                <div class="col">
                    ...
                    <xsl:if test="//t:support//t:p//text() or //t:support/t:rs[not(@type = 'decoration')]//text()">
                        ...
                        <p>
                            <xsl:apply-templates select="//t:support/t:p" mode="ogham-dimensions"/>
                            <xsl:text> </xsl:text>
                            <xsl:for-each select="//t:support/t:rs[not(@type = 'decoration')]">
                                <xsl:apply-templates select="." mode="ogham-dimensions"/>
                                <xsl:if test="position() != last()">
                                    <xsl:text>; </xsl:text>
                                </xsl:if>
                            </xsl:for-each>
                    ...
                    <xsl:if test="//t:support/t:dimensions//text()">
                        <p>
                            <b>Dimensions: </b>
                            <xsl:apply-templates select="//t:support/t:dimensions" mode="ogham-dimensions"/>
                        </p>
                    </xsl:if>
                    ...
                    <xsl:if test="//t:supportDesc//t:condition//text()">
                        <p>
                            <b>Condition: </b>
                            <xsl:apply-templates select="//t:supportDesc//t:condition"/>
                        </p>
                    </xsl:if>
              ...
              -->
                    ...
                    <div class="section-container tabs" data-section="tabs">
                        ...
                        <div class="tab-content" id="nav-tabContent">
                            <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab" tabindex="0">
                                ...
                                <!-- Wrap Ogham output with explicit font + script hint -->
                                <span class="ogham-text" lang="und-Ogam">
                ...
                <div class="col-4">
                    ...
                    <xsl:if test="//t:facsimile">
                        <!-- Filter out placeholder images like example.jpg -->
                        <xsl:variable name="graphics" select="//t:facsimile/t:graphic[@url != 'example.jpg']"/>

                        <!-- Show images block only if we have real graphics -->
                        <xsl:if test="count($graphics) &gt; 0">
                            <div id="images" class="text-center">
                                <xsl:choose>
                                    <xsl:when test="count($graphics) = 1">
                                        <xsl:variable name="g" select="$graphics[1]"/>
                                        <figure class="figure my-3 text-center">
                                            <img src="{concat('https://ogham.celt.dias.ie/images/', $g/@url)}" class="figure-img img-fluid rounded shadow-sm d-block mx-auto" title="{$g/t:desc}"/>
                                            <xsl:if test="$g/t:desc">
                                                <figcaption class="figure-caption text-center text-muted small mt-2">
                                                    <xsl:apply-templates select="$g/t:desc"/>
                                                </figcaption>
                                            </xsl:if>
                                        </figure>
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <!-- Multiple images: Bootstrap carousel with centered images -->
                                        <div id="carouselExampleCaptions" class="carousel slide my-3 custom-carousel">
                                            <div class="carousel-inner text-center">
                                                <xsl:for-each select="$graphics">
                                                    <div class="carousel-item {if (position() = 1) then 'active' else ''}">
                                                        <img src="{concat('https://ogham.celt.dias.ie/images/', @url)}" class="d-block img-fluid rounded shadow-sm mx-auto" style="max-width: 100%; height: auto;" title="{t:desc}"/>
                                                        <xsl:if test="t:desc">
                                                            <figcaption class="figure-caption text-center text-muted small mt-2">
                    ...
                    <div id="3dhop" class="tdhop">
                        ...
                        <div id="toolbar">
                            ...
                            <!-- Removed extra spacing that pushed the toolbar down -->
                            <!-- <br/> -->
                    ...
                    <!-- Tiny override to pin the toolbar near the top-left -->
                    <style>
                        /* Ensure the viewer is a positioned container */
                        #3dhop { position: relative; }
                        /* Pin toolbar to the top with minimal offset */
                        #toolbar { position: absolute; top: 8px; left: 10px; z-index: 5; }
                        /* Remove any default top margins that push content down */
                        #toolbar .icon-container { margin-top: 0 !important; }
                    </style>
                    <!-- SPIDERGL -->
                    ...
                    <br/>
                    <!-- Download link for .ply -->
                    <xsl:variable name="docid" select="normalize-space(string(//t:publicationStmt/t:idno[@type='filename']))"/>
                    <!--<xsl:if test="$docid">
                        <div class="mt-2">
                            <a class="btn btn-sm btn-outline-secondary" id="ply-download" href="/ogham_plys/{$docid}/{$docid}.ply">
    ...
    <!-- dimensions -->
    <xsl:template match="t:dimensions" mode="ogham-dimensions">
        <xsl:if test="@type">
            <xsl:value-of select="@type"/>
            <xsl:text>: </xsl:text>
        </xsl:if>
