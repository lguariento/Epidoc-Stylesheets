...
<!-- $Id$ --><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:t="http://www.tei-c.org/ns/1.0" xmlns:EDF="http://epidoc.sourceforge.net/ns/functions" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="t" version="2.0">
    ...
    <xsl:template match="t:gap[@reason='lost']">
        ...
        <xsl:if test="$parm-leiden-style= ('london','medcyprus') and preceding-sibling::node()[1][@part='M' or @part='I'] and not($parm-edition-type='diplomatic')">
            <xsl:text>-</xsl:text>
        </xsl:if>
        ...
        <xsl:if test="$parm-leiden-style= ('london','medcyprus') and following-sibling::node()[1][@part='M' or @part='F'] and not($parm-edition-type='diplomatic')">
            <xsl:text>-</xsl:text>
        </xsl:if>
    ...
    <xsl:template name="extent-string-content">
        ...
        <xsl:variable name="circa">
            <xsl:choose>
                <xsl:when test="$parm-leiden-style = ('ddbdp','dclp','sammelbuch') and                (@precision='low' or ((@unit='character' or ($parm-edn-structure='ogham' and @unit='stroke')) and number(@quantity) &gt; $cur-max))">
                    <xsl:text>ca.</xsl:text>
                </xsl:when>
        ...
        <xsl:choose>
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="$parm-leiden-style = 'sigidoc'"><!-- ++++++++++++++++++++++++++++++++++++++++++++ -->
                        <xsl:choose>
                            ...
                            <xsl:when test="@reason='illegible' and (@unit='character' or ($parm-edn-structure='ogham' and @unit='stroke'))">
                                <xsl:text>...?...</xsl:text>
                            </xsl:when>
                        </xsl:choose>
                    </xsl:when>
            ...
            <xsl:when test="@quantity and (@unit='character' or ($parm-edn-structure='ogham' and @unit='stroke'))">
                <xsl:choose>
