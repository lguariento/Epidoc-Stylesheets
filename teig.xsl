...
<!-- $Id$ --><xsl:stylesheet xmlns:EDF="http://epidoc.sourceforge.net/ns/functions" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:t="http://www.tei-c.org/ns/1.0" exclude-result-prefixes="t EDF" version="2.0">
        ...
        <xsl:choose>
            ...
            <xsl:when test="starts-with(@ref, '#') and $chardecl//t:glyph[@xml:id = $glyphID]">
                <xsl:for-each select="$chardecl//t:glyph[@xml:id = $glyphID]">
                    <xsl:choose>
                        <xsl:when test="$parm-leiden-style = 'ogham' and $g[ancestor::t:div[@subtype='ogham']]">
                            <span>
